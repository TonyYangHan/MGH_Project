...
test_ad_donor = cv_cfg.get("ad_donor", "")
test_control_donor = cv_cfg.get("control_donor", "")
if not isinstance(test_ad_donor, str) or not test_ad_donor:
    raise ValueError("cross_validation.ad_donor must be a non-empty string")
if not isinstance(test_control_donor, str) or not test_control_donor:
    raise ValueError("cross_validation.control_donor must be a non-empty string")
...
# donor pairing
donors = adata.obs[donor_key].astype(str)
ad_donors = donors.loc[adata.obs[label_key] == ad_label].unique()
ctrl_donors = donors.loc[adata.obs[label_key] == control_label].unique()
if test_ad_donor not in ad_donors:
    raise ValueError(
        f"Configured AD donor {test_ad_donor!r} was not found. "
        f"Available AD donors: {sorted(ad_donors)}"
    )
if test_control_donor not in ctrl_donors:
    raise ValueError(
        f"Configured control donor {test_control_donor!r} was not found. "
        f"Available control donors: {sorted(ctrl_donors)}"
    )

selected_test_pairs = np.array([[test_ad_donor, test_control_donor]], dtype=object)
validation_folds = selected_test_pairs.shape[0]
...
for i in trange(validation_folds, desc="Folds"):
    test_donors = selected_test_pairs[i]
    is_test = donors.isin(test_donors)
...
metrics_df = pd.DataFrame({
    ...
    'AD_donor': selected_test_pairs[:, 0],
    'Ctrl_donor': selected_test_pairs[:, 1],
    'AUROC': aucs,
    'Accuracy': accs,
    'threshold': thrs
})
